                ...
                <TimelineMarkersProvider>
                    ...
                    >
                        <div
                            ...
                            className="anteros-calendar-timeline"
                        >
                            {this.renderHeaders()}
    ...
    render() {
        ...
        return (
            <CustomHeaderWrapper
                ...
                children={CustomDateHeaderWrapper}
            />
        )
    }
}
...
const { TimelineHeadersConsumer, _TimelineHeadersConsumer } = createReactContext({
    registerScroll: () => {
...
class TimelineHeadersProvider extends React.Component {
    ...
    render() {
        ...
        return <_TimelineHeadersConsumer value={contextValue}>{this.props.children}</_TimelineHeadersConsumer>
    }
}
...
class _AnterosSidebarHeader extends React.PureComponent {
    getRootProps(props = {}) {
        const { style } = props
        const width =
...
_AnterosSidebarHeader.propTypes = {
    children: PropTypes.func.isRequired,
    rightSidebarWidth: PropTypes.number,
    leftSidebarWidth: PropTypes.number.isRequired,
...
class _AnterosTimelineHeaders extends React.Component {
    constructor(props) {
        super(props)
    }
    ...
    render() {
        ...
        if (!leftSidebarHeader) {
            leftSidebarHeader = <SidebarHeaderWrapper />
        }
        if (!rightSidebarHeader && this.props.rightSidebarWidth) {
            rightSidebarHeader = <SidebarHeaderWrapper variant="right" />
        }
        return (
            <div
...
_AnterosTimelineHeaders.propTypes = {
    registerScroll: PropTypes.func.isRequired,
    leftSidebarWidth: PropTypes.number.isRequired,
...
class _AnterosMarkerCanvas extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
...
_AnterosMarkerCanvas.propTypes = {
...
const { MarkerCanvasConsumer, MarkerCanvasProvider } = createReactContext({
    subscribeToMouseOver: () => {
        console.warn('"subscribeToMouseOver" default func is being used')
    }
});

const { TimelineMarkersConsumer, _TimelineMarkersProvider } = createReactContext({
    markers: [],
    subscribeMarker: () => {
...
class TimelineMarkersProvider extends React.Component {
    ...
    render() {
        return <_TimelineMarkersProvider value={this.state}>{this.props.children}</_TimelineMarkersProvider>
    }
}
...
class _AnterosCursorMarkerImpl extends React.Component {
    componentDidMount() {
...
_AnterosCursorMarkerImpl.propTypes = {
...
const AnterosCursorMarkerImpl = props => {
    return (
        <TimelineMarkersConsumer>
            {({ subscribeMarker }) => (
                <_AnterosCursorMarkerImpl subscribeMarker={subscribeMarker} {...props} />
            )}
        </TimelineMarkersConsumer>
    )
}
...
class _AnterosCustomMarkerImpl extends React.Component {
    componentDidUpdate(prevProps) {
...
_AnterosCustomMarkerImpl.propTypes = {
    subscribeMarker: PropTypes.func.isRequired,
...
class _AnterosTodayMarker extends React.Component {
    componentDidMount() {
        const { unsubscribe, getMarker } = this.props.subscribeMarker({
...
_AnterosTodayMarker.defaultProps = {
    interval: 1000 * 10
}
...
const AnterosTodayMarker = props => {
    return (
        <TimelineMarkersConsumer>
            {({ subscribeMarker, updateMarker }) => (
                <_AnterosTodayMarker subscribeMarker={subscribeMarker} updateMarker={updateMarker} {...props} />
            )}
        </TimelineMarkersConsumer>
    )
}
...
const { TimelineStateConsumer, _TimelineStateProvider } = createReactContext({
    getTimelineState: () => {
...
export class TimelineStateProvider extends React.Component {
    ...
    render() {
        return (
            <_TimelineStateProvider value={this.state.timelineContext}>
                {this.props.children}
            </_TimelineStateProvider>
        )
    }
}
...
class _AnterosCursorMarker extends React.Component {
    componentDidMount() {
        const { unsubscribe } = this.props.subscribeMarker({
            type: TimelineMarkerType.Cursor,
            renderer: this.props.children
...
_AnterosCursorMarker.propTypes = {
    subscribeMarker: PropTypes.func.isRequired,
    children: PropTypes.func
}
...
class _AnterosCustomMarker extends React.Component {
    componentDidUpdate(prevProps) {
        if (prevProps.date !== this.props.date && this.getMarker) {
            const marker = this.getMarker()
            this.props.updateMarker({ ...marker, date: this.props.date })
...
_AnterosCustomMarker.propTypes = {
    subscribeMarker: PropTypes.func.isRequired,
    updateMarker: PropTypes.func.isRequired,
    children: PropTypes.func,
    date: PropTypes.number.isRequired
