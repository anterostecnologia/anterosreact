...
import { getPageIdx, getNodeIdxClassName, BasicType, AdvancedType, getNodeTypeFromClassName, JsonToMjml, createBlockDataByType, BlockManager, getIndexByIdx, getParentByIdx, getParentIdx, getValueByIdx, getChildIdx, getNodeIdxFromClassName, MERGE_TAG_CLASS_NAME, getSameParent } from "@anterostecnologia/anteros-email-core";
...
const EASY_EMAIL_EDITOR_ID = "anteros-email-editor";
const PLUGINS_CONTAINER_ID = "anteros-email-plugins";
const SYNC_SCROLL_ELEMENT_CLASS_NAME = "anteros-email-sync-scroll";
const RICH_TEXT_BAR_ID = "anteros-email-rich-text-bar";
...
const CONTENT_EDITABLE_CLASS_NAME = "anteros-email-content_editable_text_only";
const CONTENT_EDITABLE_RICH_TEXT_CLASS_NAME = "anteros-email-content_editable_rich_text";
...
const transform = (text, id) => {
  return text.replace(/{{([\s\S]+?)}}/g, (_, $1) => {
    ...
    input.className = "anteros-email-merge-tag";
    input.value = $1;
    input.type = "button";
    if (id) {
      input.id = id;
...
class MergeTagBadge {
  ...
  static revert(content, generateMergeTag) {
    const container = document.createElement("div");
    container.innerHTML = content;
    container.querySelectorAll(".anteros-email-merge-tag").forEach((item) => {
      var _a;
...
function useBlock() {
  ...
  const addBlock = useCallback((params) => {
    ...
    if (!parent) {
      console.error(`Bloco ${type} inv\xE1lido`);
      return;
    }
    ...
    if (!(fixedBlock == null ? void 0 : fixedBlock.validParentType.includes(parent.type))) {
      console.error(`${block.type} n\xE3o pode ser usado dentro ${parentBlock.type}, apenas dentro: ${block.validParentType.join(", ")}`);
      return;
    }
  ...
  const moveBlock = useCallback((sourceIdx, destinationIdx) => {
    ...
    if (autoComplete) {
      ...
      if (autoCompletePaths) {
        ...
        console.error("Algo quando errado");
      }
    }
  ...
  const copyBlock = useCallback((idx) => {
    ...
    if (!parent) {
      console.error("Bloco inv\xE1lido");
      return;
    }
  ...
  const removeBlock = useCallback((idx) => {
    ...
    if (!parentIdx || !parent) {
      if (block.type === BasicType.PAGE) {
        console.error("O n\xF3 da p\xE1gina n\xE3o pode remover");
        return;
      }
      console.error("Bloco inv\xE1lido");
      return;
...
const BlockAvatarWrapper = (props) => {
  ...
  const onDragStartProgramatically = useCallback(() => {
    if (action === "add") {
      setDataTransfer({
        type,
        action,
  ...
  useEffect(() => {
    ...
    ele.addEventListener("dragstart", onDragStartProgramatically);
    return () => {
      ele.removeEventListener("dragend", onDragEnd);
      ele.removeEventListener("dragstart", onDragStartProgramatically);
    };
  }, [onDragEnd, onDragStartProgramatically]);
...
const Button = (props) => {
  return /* @__PURE__ */ React.createElement("button", {
    onClick: props.onClick,
    className: classnames("anteros-email-editor-button", props.noBorder && "anteros-email-editor-noBorder"),
    title: props.title,
...
const SyncScrollIframeComponent = (_a) => {
  var _b = _a, { children, title, windowRef } = _b, props = __objRest(_b, ["children", "title", "windowRef"]);
  ...
  const onLoad = (evt) => {
    var _a2;
    const contentWindow2 = (_a2 = evt.target) == null ? void 0 : _a2.contentWindow;
    if (!contentWindow2)
      return;
...
function PreviewEmail(props) {
  ...
  if (isMobile) {
    return /* @__PURE__ */ React.createElement(SyncScrollIframeComponent, {
      style: {
        border: "none",
        height: "100%",
        width: "100%"
  ...
  return /* @__PURE__ */ React.createElement(SyncScrollShadowDom, {
...
function MobileEmailPreview() {
  ...
  const { activeTab } = useActiveTab();
  return /* @__PURE__ */ React.createElement("div", {
    className: "anteros-email-overlay",
    style: {
      height: "100%",
      display: "flex",
      alignItems: "center",
    ...
    isActive: activeTab === ActiveTabKeys.MOBILE
  })))));
}
...
const Tabs = (props) => {
  ...
  return /* @__PURE__ */ React.createElement("div", {
    ...
    className: "anteros-email-editor-tabWrapper"
    ...
    return /* @__PURE__ */ React.createElement("div", {
      ...
      className: classnames("anteros-email-editor-tabItem", !activeTab && index2 === 0 && "anteros-email-editor-tabActiveItem", activeTab === item.key && "anteros-email-editor-tabActiveItem")
    }, /* @__PURE__ */ React.createElement(Button, {
      noBorder: true
    }, item.props.tab));
