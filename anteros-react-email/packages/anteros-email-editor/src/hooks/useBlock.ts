...
export function useBlock() {
  ...
  const addBlock = useCallback(
    (params: {
      ...
      if (!parent) {
        console.error(`Bloco ${type} inválido`);
        return;
      }
      ...
      if (!fixedBlock?.validParentType.includes(parent.type)) {
        console.error(
          `${block.type} não pode ser usado dentro ${
            parentBlock.type
          }, apenas dentro: ${block.validParentType.join(', ')}`
        );
        return;
      }
    ...
    (idx: string) => {
      ...
      if (!parentIdx || !parent) {
        if (block.type === BasicType.PAGE) {
          console.error('O nó da página não pode remover');
          return;
        }
        console.error('Bloco inválido');
        return;
