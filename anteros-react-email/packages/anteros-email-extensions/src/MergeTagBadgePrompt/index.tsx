...
const removeAllActiveBadge = () => {
  getShadowRoot()
    .querySelectorAll('.anteros-email-merge-tag')
    .forEach((item) => {
      item.classList.remove('anteros-email-merge-tag-focus');
    });

  const popoverNode = getShadowRoot().querySelectorAll(
    '.anteros-email-merge-tag-popover'
  );
  if (popoverNode) {
  }
};
  ...
  useEffect(() => {
    ...
    const onClick: EventListenerOrEventListenerObject = (e) => {
      ...
      if (
        ...
        target.classList.contains('anteros-email-merge-tag')
      ) {
        target.classList.add('anteros-email-merge-tag-focus');
  ...
  return (
    <>
      ...
      {textContainer && createPortal(
        <div ref={popoverRef} onClick={onClick} className={classnames('anteros-email-merge-tag-popover')}>
          <div className='anteros-email-merge-tag-popover-container'>
            <h3>
              <span>Valor padrão</span>
              <IconFont style={{ color: 'rgb(92, 95, 98)' }} iconName='icon-close' onClick={onClose} />
            </h3>
            <div className={'anteros-email-merge-tag-popover-desc'}>
              <p>
                Se um valor de texto personalizado não estiver disponível, um valor padrão
                é mostrado.
              </p>
              <div className='anteros-email-merge-tag-popover-desc-label'>
                <input autoFocus value={text} onChange={onChange} type="text" autoComplete='off' maxLength={40} />
                <div className='anteros-email-merge-tag-popover-desc-label-count'>
                  {text.length}/40
                </div>
              </div>
              <div className='anteros-email-merge-tag-popover-desc-label-button'>
