...
import { AdvancedBlock, OperatorSymbol, AdvancedType, Operator, ICondition, IConditionGroup } from '@anterostecnologia/anteros-email-core';
...
export function Condition() {
  ...
  const onDelete = useCallback((path: string, gIndex: number, ggIndex: number) => {
    ...
    if (subGroups.length === 0) {
      if (groups.length === 1) {
        Message.warning('Pelo menos uma condição');
        return;
      }
      // remove empty array
...
function ConditionItem({ path, onDelete, gIndex, ggIndex }: { path: string; gIndex: number; ggIndex: number; onDelete: (path: string, gIndex: number, ggIndex: number,) => void; }) {
  ...
  return (
    <Grid.Row align='end'>
      <Grid.Col span={7}> <TextField label="Caminho variável" name={`${name}.left`} /></Grid.Col>
      <Grid.Col span={7}> <SelectField label="Operador" name={`${name}.operator`} options={options} /></Grid.Col>
