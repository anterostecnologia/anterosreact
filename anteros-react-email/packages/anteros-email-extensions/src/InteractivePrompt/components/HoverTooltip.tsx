...
import { getNodeTypeFromClassName, BlockManager } from '@anterostecnologia/anteros-email-core';
...
import { useLazyState } from '@anterostecnologia/anteros-email-editor';
...
export function HoverTooltip() {
  ...
  return (
    <>
      {createPortal(
        <div
          id='anteros-email-extensions-InteractivePrompt-HoverTooltip'
          style={{
            position: 'absolute',
            height: '100%',
            width: '100%',
...
function TipNode(props: TipNodeProps) {
  ...
  const dragTitle = useMemo(() => {
    if (direction === 'top' || direction === 'noEnoughTop') {
      return `Inserir antes de ${title}`;
    } else if (direction === 'bottom') {
      return `Inserir após ${title}`;
    } else if (direction === 'right' || direction === 'left') {
      return 'Arraste aqui';
    }
    return `Arraste para ${title}`;
  }, [direction, title]);
