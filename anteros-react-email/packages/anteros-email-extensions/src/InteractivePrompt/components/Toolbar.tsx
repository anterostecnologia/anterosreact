...
import { BasicType, getParentIdx, getSiblingIdx } from '@anterostecnologia/anteros-email-core';
import {
  useBlock,
  useFocusIdx,
  useEditorProps,
...
export function Toolbar() {
  ...
  const handleCopy: React.MouseEventHandler<HTMLDivElement> = (ev:any) => {
    if (document.activeElement instanceof HTMLElement) {
      document.activeElement.blur();
    }
    copyBlock(focusIdx);
  ...
  return (
    <>
      <div
        id='anteros-email-extensions-InteractivePrompt-Toolbar'
        style={{
          height: 0,
          zIndex: 100,
        }}
