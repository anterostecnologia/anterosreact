...
- **`stateHistory`** - an object containing history state:
    ...
    - **`position`**_``_ - current position _index_ in history;
    - **`capacity`**_` = 10`_ - maximum amount of history entries;
    - **`back`**_`: (amount?) => void`_ - go back in state history, it will cause `setState` to be invoked and component re-render.
    If first element of history reached, the call will have no effect;
    - **`forward`**_`: (amount?) => void`_ - go forward in state history, it will cause `setState` to be invoked and component re-render.  
    If last element of history is reached, the call will have no effect;
    - **`go`**_`: (position) => void`_ - go to arbitrary position in history.  
