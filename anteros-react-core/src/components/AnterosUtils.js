...
class AnterosUtils {
	...
	parseNumber(value) {
		...
		if ((value.match(new RegExp("\\" + decimal, "g")) || []).length > 1)
			decimal = "";
		if (decimal != "" && value.length - value.indexOf(decimal) - 1 == 3)
			decimal = "";
	...
	formatNumber(value, mask) {
		...
		// process the first group separator from decimal (.) only, the rest ignore. get
		// the length of the last slice of split result.
		...
		// put back any negation, combine integer and fraction, and add back prefix &
		// suffix
	...
	// https://github.com/twbs/bootstrap/blob/v4.0.0-alpha.4/js/src/modal.js#L436-L4
	// 43
	getScrollbarWidth() {
		let scrollDiv = document.createElement("div");
		// .modal-scrollbar-measure styles //
		// https://github.com/twbs/bootstrap/blob/v4.0.0-alpha.4/scss/_modal.scss#L106-L1
		// 13
		...
		let ids = items
			...
			.map(item => {
				childrenIds = childrenIds.concat(
					this.getAllNonEmptyNodesIds(item[childrenProp], childrenProp)
				);
				return item.id;
			});
	...
	scrollToComponent(ref, options) {
		...
		return this.scrollTo(
			0,
			this.calculateScrollOffset(element, options.offset, options.align),
			options
		);
	}
