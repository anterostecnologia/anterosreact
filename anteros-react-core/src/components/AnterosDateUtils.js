...
var formatters = {
	...
	DDDD: function(date) {
		return new AnterosDateUtils().addLeadingZeros(
			new AnterosDateUtils().getDayOfYear(date),
			3
		);
	},
	...
	WW: function(date) {
		return new AnterosDateUtils().addLeadingZeros(
			new AnterosDateUtils().getISOWeek(date),
			2
		);
	},
...
class AnterosDateUtils {
	...
	differenceInHours(dirtyDateLeft, dirtyDateRight) {
		var diff =
			this.differenceInMilliseconds(dirtyDateLeft, dirtyDateRight) /
	...
	differenceInISOYears(dirtyDateLeft, dirtyDateRight) {
		...
		var sign = this.compareAsc(dateLeft, dateRight);
		var difference = Math.abs(
			this.differenceInCalendarISOYears(dateLeft, dateRight)
		);
	...
	differenceInSeconds(dirtyDateLeft, dirtyDateRight) {
		var diff =
			this.differenceInMilliseconds(dirtyDateLeft, dirtyDateRight) / 1000;
		return diff > 0 ? Math.floor(diff) : Math.ceil(diff);
	}
	...
	differenceInYears(dirtyDateLeft, dirtyDateRight) {
		...
		var sign = this.compareAsc(dateLeft, dateRight);
		var difference = Math.abs(
			this.differenceInCalendarYears(dateLeft, dateRight)
		);
	...
	formatDate(dirtyDate, dirtyFormatStr, dirtyOptions) {
		...
		var formattingTokensRegExp = this.buildFormatLocale()
		...
		var formatFn = this.buildFormatFn(
			formatStr,
			localeFormatters,
			formattingTokensRegExp
		);
	...
	formatTimezone(offset, delimeter) {
		...
		return (
			sign +
			this.addLeadingZeros(hours, 2) +
			delimeter +
			this.addLeadingZeros(minutes, 2)
		);
	}
	...
	getISOWeek(dirtyDate) {
		var date = this.parse(dirtyDate);
		var diff =
			this.startOfISOWeek(date).getTime() - this.startOfISOYear(date).getTime();
	...
	isToday(dirtyDate) {
		return (
			this.startOfDay(dirtyDate).getTime() ===
	...
	isTomorrow(dirtyDate) {
		...
		return (
			this.startOfDay(dirtyDate).getTime() ===
			this.startOfDay(tomorrow).getTime()
		);
	}
	...
	isYesterday(dirtyDate) {
		...
		return (
			this.startOfDay(dirtyDate).getTime() ===
			this.startOfDay(yesterday).getTime()
		);
	}
	...
	parseTimezone(timezoneString) {
		...
		if (token) {
			absoluteOffset =
				this.parseInt(token[2], 10) * 60 + this.parseInt(token[3], 10);
			return token[1] === "+" ? -absoluteOffset : absoluteOffset;
		}
	...
	parseDateWithFormat(value, format) {
		...
		while (i_format < format.length) {
			...
			if (
				token.toLowerCase() == "yyyy" ||
				token.toLowerCase() == "yy" ||
				token.toLowerCase() == "y"
			) {
				...
				for (var i = 0; i < MONTH_NAMES.length; i++) {
					...
					if (
						value.substring(i_val, i_val + month_name.length).toLowerCase() ==
				...
				for (var i = 0; i < DAY_NAMES.length; i++) {
					...
					if (
						value.substring(i_val, i_val + day_name.length).toLowerCase() ==
