    ...
    componentWillReceiveProps(nextProps) {
        this.setState({
            ...this.state, expanded: (nextProps.node.state && nextProps.node.state.hasOwnProperty('expanded')) ?
                ...
                false,
            selected: (nextProps.node.state && nextProps.node.state.hasOwnProperty('selected')) ?
                nextProps.node.state.selected :
                false,
            loading: (nextProps.node.state && nextProps.node.state.hasOwnProperty('loading')) ?
    ...
    render() {
        ...
        if (options.selectable) {
            if (this.state.selected) {
                checkSelectedIcon = (
                    <i style={iconStyle} className="fad fa-check-square"
                        onClick={this.toggleSelected}> </i>
                )
            } else {
                checkSelectedIcon = (
                    <i style={iconStyle} className="far fa-square"
                        onClick={this.toggleSelected}> </i>)
            }
        }
        ...
        if (!this.state.expanded) {
            expandCollapseIcon = (
                <i style={{...iconStyle,width:"1rem"}} className="far fa-chevron-right"
            ...
            if (this.props.node.state.loading === true) {
                expandCollapseIcon = (
                    <i style={{...iconStyle,width:"1rem"}} className="svg-inline--fa fa-spinner fa-w-16 fa-spin fa-lg"></i>
                )
            } else {
                expandCollapseIcon = (
                    <i style={{...iconStyle,width:"1rem"}} className="far fa-chevron-down"
                        onClick={this.toggleExpanded}>   </i>
                )
            }
        }
        ...
        if (node.icon) {
            icon = (
                <i style={{...iconStyle,padding:"0.2rem", color: node.color}} className={node.icon}></i>
            );
        }
        ...
        if (children.length > 0 || !(this.state.node.isleaf)) {
            return (
                <li style={style} onDoubleClick={this.doubleClicked} key={options.id + "_" + node.id}>
                    {expandCollapseIcon}
                    {checkSelectedIcon}
                    <span style={styleFocused} className="anteros-treeview-item" onClick={this.toggleFocused}>{icon}{node.image?<img alt="" style={{padding:"0.2rem"}} src={node.image}/>:null}{node.text}</span>
                    <ul>
                        {children}
                    </ul>
                </li>
