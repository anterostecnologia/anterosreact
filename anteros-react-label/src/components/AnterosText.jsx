...
export default class AnterosText extends Component {
  ...
  render() {
    ...
    if (colClasses.length > 0) {
      _className = AnterosUtils.buildClassNames(
        className ? className : "",
        "control-label",
        colClasses,
    ...
    if (maskPattern === "datetime") {
      let dt = AnterosDateUtils.parseDateWithFormat(
        text,
        Anteros.dataSourceDatetimeFormat
      );
      if (dt instanceof Date) {
        newText = AnterosDateUtils.formatDate(
          dt,
          Anteros.displayDatetimeFormat
        );
      }
