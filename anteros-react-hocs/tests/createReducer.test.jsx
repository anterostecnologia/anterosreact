...
import createReducer from '../src/hooks/createReducer';

it('deveria iniciar a função reducer hook', () => {
  const useSomeReducer = createReducer();
  expect(useSomeReducer).toBeInstanceOf(Function);
});

describe('usar ao criar reducer hook', () => {
  ...
  // Criador de ação para incrementar a contagem, espere um segundo e então reinicie
  const addAndReset = () => {
    return dispatch => {
  ...
  it('deve lidar com ação assíncrona com vários middlewares', () => {
    const { result } = setUp();
    const [, dispatch] = result.current;
    expect(mockLog).not.toHaveBeenCalled();
    ...
    // avançar até que todos os cronômetros sejam executados
    act(() => {
      jest.runAllTimers();
    });
