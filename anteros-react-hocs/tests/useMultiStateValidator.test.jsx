...
import { useMultiStateValidator } from '../src/hooks/useMultiStateValidator';
describe('useMultiStateValidator', () => {
    it('should be defined', () => {
        expect(useMultiStateValidator).toBeDefined();
    });
    const defaultStatesValidator = (states) => [states.every(num => !(num % 2))];
    function getHook(fn = jest.fn(defaultStatesValidator), initialStates = [1, 2], initialValidity = [false]) {
        return [
            fn,
            renderHook(({ initStates, validator, initValidity }) => {
    ...
    it("should NOT call validator on it's change", () => {
        const [spy, hook] = getHook();
        const newValidator = jest.fn(states => [states.every(num => !!(num % 2))]);
        expect(spy).toHaveBeenCalledTimes(1);
