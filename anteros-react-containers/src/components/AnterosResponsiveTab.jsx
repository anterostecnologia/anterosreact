...
class AnterosResponsiveTabs extends Component {
  constructor(props) {
    ...
    this.onResizeThrottled = lodash.throttle(
      this.onResize,
      props.resizeThrottle,
      { trailing: true }
    );
    autoBind(this);
  }
  ...
  getTabs(items) {
    ...
    const { blockWidth, tabsTotalWidth, tabDimensions, showMoreWidth } =
      this.state;
  ...
  render() {
    ...
    const { tabsVisible, tabsHidden, panels, isSelectedTabHidden } =
      this.getTabs(this.getItems());
    const isCollapsed = this.getIsCollapsed();
    const selectedTabKey = this.getSelectedTabKey();
    ...
    const tabsClasses = classNames("AnterosTab_tabs", tabsWrapperClass, {
      AnterosTab_accordion: isCollapsed,
    });
...
class ShowMore extends Component {
  ...
  render() {
    const { isShown, children, onShowMoreChanged, hasChildSelected, label } =
      this.props;
    const { isHidden } = this.state;
