...
const checkOnMountAndUnmount = (props, addEventListenerName, removeEventListenerName) => {
  const { unmount } = renderHook((p) => useEvent(p.name, p.handler, p.target, p.options), {
    initialProps: props,
  });
...
const checkOnDepsChanges = (
  ...
  const { rerender } = renderHook((p) => useEvent(p.name, p.handler, p.target, p.options), {
    initialProps: props1,
  });
