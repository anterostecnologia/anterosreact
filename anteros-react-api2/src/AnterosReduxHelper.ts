...
export function createReducer(_initialState, _reducer) {
...
export function makeDefaultReduxObject(_reducerName) {
  return {
    ...
    [`SET_${_reducerName.toUpperCase()}_NEEDREFRESH`]: (state, payload) => ({
      ...state,
      needRefresh: true,
    }),
  };
...
export function makeDefaultReduxActions(_actionName) {
  return {
