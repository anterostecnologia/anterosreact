...
export default class AnterosLookupCombobox extends React.Component {
	constructor(props) {
		...
		if (this.props.dataSource) {
			...
			if (value) value = value[this.props.dataFieldId];
		}
		this.state = { value, update: true };

		if (
			this.props.lookupDataSource &&
			this.props.lookupDataSource.locate({
				[`window.${this.props.lookupDataFieldId}`]: this.state.value
			})
		) {
			if (this.props.dataSource) {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (nextProps.dataSource) {
			...
			if (value) {
				if (typeof nextProps.lookupDataFieldText === "function") {
					value = nextProps.lookupDataFieldText(
						nextProps.dataSource.getCurrentRecord()
					);
				} else {
			...
			children.push(
				React.createElement(
					AnterosLookupComboboxOption,
					{
						...
						label: record.label
							? record.label
							: _this.getTextValue(record),
						text: _this.getTextValue(record),
						group: record.group,
...
AnterosLookupCombobox.propTypes = {
	...
	lookupDataFieldText: PropTypes.oneOfType([
        PropTypes.string,
        PropTypes.func
      ]),
