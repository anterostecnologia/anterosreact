...
export default class AnterosCombobox extends Component {
  ...
  buildOptions(props) {
    if (props.options) {
      ...
      this._flatOptions = this.flattenOptions(
        this.rebuildOptions(props.children)
      );
    }
  }
  ...
  handleInputFocus(event) {
    if (
      this.props.disabled ||
      (this.props.dataSource && this.props.dataSource.getState() === "dsBrowse")
    )
      return;
    ...
    if (this.props.onChangeSelect) {
      if (this.props.multi) {
        ...
        value.forEach((item) => {
          ...
          result += item[this.props.valueKey];
          appendDelimiter = true;
        });
        this.props.onChangeSelect(result);
      } else {
        this.props.onChangeSelect(value[this.props.valueKey]);
      }
    }
  }
      ...
      if (child.type && child.type.componentName === "AnterosComboboxOption") {
        if (child.group && child.group === true) {
          result.push({
            ...
            options: this.rebuildOptions(child.props.children),
          });
        } else {
          result.push({
            label: child.props.label,
  ...
  render() {
    ...
    let dataStyle = { ...this.props.style, width, height };
    ...
    let content = (
      <div
        ...
        className={className}
        style={{ ...this.props.wrapperStyle, width, height }}
      >
